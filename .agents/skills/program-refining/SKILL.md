...
- Read one epic under `plans/epics/{epic_name}/` as broad source context
...
- Write durable refinement artifacts back into the epic folder so later agents can continue without rereading the whole epic
...
Use this after `program-planning` when work now exists as one or more epics under `plans/epics/`.
...
1. Read `plans/epics/{epic_name}/00-overview.md`.
2. Read only the relevant initiative docs under `initiatives/{initiative_name}/`.
3. Read only the relevant program docs under `docs/specs/{program_name}/` when they materially constrain the epic.
4. Read direct prerequisite active plans under `plans/` only when they affect slice boundaries.
5. Read direct prerequisite completed plans under `plans/completed/` only when they affect what is already done or what constraints must carry forward.
...
Write or update these files under `plans/epics/{epic_name}/`:
...
- broad epic context: `plans/epics/{epic_name}/`
