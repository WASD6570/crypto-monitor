...
test('loads dashboard data from the Go market-state API without frontend route mocks', async ({ page }) => {
  ...
  await expect(page.getByTestId('section-slot-overview')).toContainText(/BTC-USD is /)
  ...
  await expect(page.getByTestId('section-slot-overview')).toContainText(/ETH-USD is /)
})
