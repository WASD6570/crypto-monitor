...
4. If `usdmStatus` is present, record `usdmStatus.websocket`, `usdmStatus.openInterest`, `lastMarkPriceAt`, `lastOpenInterestAt`, `nextOpenInterestPollAt`, and `openInterestRateLimitUntil`.
5. If you need the numeric lag or rate counters, inspect the companion metrics source for `message_lag_ms`, `snapshot_lag_ms`, `reconnect_count`, `resync_count`, `sequence_gap_count`, `snapshot_recovery_attempts_per_minute`, `rest_poll_attempts_per_minute`, and `clock_offset_ms`.
6. Confirm whether the condition is warm-up (`readiness=NOT_READY`) or a readable runtime degradation (`readiness=READY` with `DEGRADED` or `STALE` feed health).
7. Confirm whether the current condition is bounded by retry policy or requires operator intervention.
...
- Confirm `usdmStatus` has returned to `HEALTHY` or keep the remaining derivatives-context issue visible in handoff notes.
